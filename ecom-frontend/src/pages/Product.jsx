...
const Product = () => {
  ...
  // 2. The Search Engine (Moved inside useEffect to avoid lint warnings)
  useEffect(() => {
    ...
    // 3. Re-run this search if the URL (productId) or the master list (products) ever changes
  }, [productId, products]);

  // Temporary safety check so the page doesn't crash while loading
  if (!productData) {
          ...
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {
              productData.image.map((item, index) => (
                <img
                  ...
                  src={item} key={index} className='w-[24%] sm:w-full sm:mb-3 shrink-0 cursor-pointer' />
              ))
            }
          </div>
